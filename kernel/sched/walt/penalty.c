...
static int penalize_yield( u64 *delta, struct walt_task_struct *wts)
{
	if(sysctl_sched_auto_penaty & ENABLE_SCHED){
		...
		{
			*delta = (delta_prev*wtsp->penaty_scale) >> SCHED_CAPACITY_SHIFT;
	...
	unsigned int *pneed_cpus)
{
	...
	for(i=0; i<data_len; i++)
	{
		c = &data[i];
		rq = cpu_rq(c->cpu);
		estimate_nr_task = rq->nr_switches - per_cpu(switch_count, c->cpu);
	...
	struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if(target_fps>0)
	{
		...
		pr_info("sched_penalty:fps=%lu, slow_thresh=%lu max_fps_drop=%d \n",
		target_fps, slow_thresh, max_fps_drop);
